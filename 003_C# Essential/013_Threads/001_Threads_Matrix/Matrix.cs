...
namespace _001_Threads_Matrix
{
    class Matrix
    {
        ...
        private char GetChar()//Метод возвращающий одно значение из строкового поля litters
        {
            return litters.ToCharArray()[rand.Next(0, 35)]; //Возвращает одно значение из строкового поля litters
        }
        ...
        public void Move()//Метод отображения одной цепочки
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < 40; i++) //Цикл со счетчиком (40 - это наша высота окна)
                {
                    lock (locker) //Блокирует блок кода, предназначен для контроля доступа к критической секции
                    {
                        ...
                        Console.ForegroundColor = ConsoleColor.Black; //Устанавливает цвет (шрифта. - дальше символ будет интересный: "█")
                        for (int j = 0; j < i; j++)
                        {
