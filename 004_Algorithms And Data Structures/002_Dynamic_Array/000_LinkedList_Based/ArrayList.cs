...
namespace _000_LinkedList_Based
{
    ...
    {
        LinkedList<T> _array;           // это вместо T[] _array; - то есть вместо статического массива - если я правильно понял задание.
        ...
        {
            _array = new LinkedList<T>();
        }

        // ctor with parameters                      //ICollection<T> здесь нужен только потому, что в нём есть св-во Count, с помощью которого мы можем установить размер внутреннего массива ArrayList. Т.о. здесь можно использовать любой тип, лишь бы в нём был Count. Или, если нет Count, то, если это массив, то использовать Length, либо, если список - то добавить в него св-во Count.
        public ArrayList(IEnumerable<T> list)
        {
            _array = new LinkedList<T>();
        ...
        // The "Item" Property  -   indexer for access to each element
        ...
        // Method IndexOf - for search element and returning its index
        ...
        // Method Contains - predicate for determining of existance of element
