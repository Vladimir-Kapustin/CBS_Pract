...
namespace _001_Queue_Priorities
{
    class LinkedList<T> : IEnumerable<T>
    {
        ...
        public void Add(T value, T priority)
        {
            LinkedListNode<T> node = new LinkedListNode<T>(value, priority);
        ...
        public void AddAfter(T[] values, T [] priorities, int position)     //в этот метод добавил ещё один массив с приоритетами. Он должен быть такого же размера как и первый входящий массив со значениями - чтоб значению соответствовал приоритет в принимающем узле. Здесь даже наверное было бы удобнее, чтобы добавлялся не целый массив - а добавлять по одному значению (значение + приоритет)
        {
            if (_head == null)
            {
            ...
            {                                       // здесь конечно можно ещё было проверить на выход индекса за пределы диапазона списка - но это уже нюансы, я делать не буду за неимением времени.
                ...
                {
                    LinkedListNode<T> item = new LinkedListNode<T>(values[i], priorities[i]);    //создаём узел для нового элемента и пишем в него входящее значение этого элемента
        ...
        public void AddFirst(T value, T priority)
        {
            LinkedListNode<T> current = new LinkedListNode<T>(value, priority);
        ...
        public void AddLast(T value, T priority)
        {
            LinkedListNode<T> current = new LinkedListNode<T>(value, priority);
        ...
        public T this[int index]
        {
            ...
            get
            {
                ...
                {
                    throw new IndexOutOfRangeException("You inserted value out of range");
                }
